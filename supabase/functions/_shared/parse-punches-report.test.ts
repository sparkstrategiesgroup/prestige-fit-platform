...
//
// ePay exports each store's local wall-clock punch time with no timezone. They
// are stored in the timestamptz columns time_in/time_out and read back by
...
Deno.test("Central, summer (CDT, UTC-5): 5:00am local -> 10:00Z", () => {
  assertEquals(
    wallClockToUtcIso(2026, 6, 22, 5, 0, 0, "America/Chicago"),
    "2026-06-22T10:00:00.000Z",
  );
});

Deno.test("Central, winter (CST, UTC-6): 5:00am local -> 11:00Z", () => {
  assertEquals(
    wallClockToUtcIso(2026, 1, 15, 5, 0, 0, "America/Chicago"),
    "2026-01-15T11:00:00.000Z",
  );
});

Deno.test("Eastern, summer (EDT, UTC-4): 2:30pm local -> 18:30Z", () => {
  assertEquals(
...
Deno.test("Arizona (no DST, UTC-7): 9:00am local -> 16:00Z", () => {
  assertEquals(
...
Deno.test("early-morning punch maps to same calendar day in UTC", () => {
  assertEquals(
    wallClockToUtcIso(2026, 6, 22, 3, 30, 0, "America/Chicago"),
    "2026-06-22T08:30:00.000Z",
  );
});

Deno.test("midnight local maps to the correct instant (no '24' artifact)", () => {
  assertEquals(
